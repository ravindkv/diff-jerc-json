...
def main(root_file_path, output_file_path):
    ...
    hist_pt_dir = root_file.Get("HistGivenPt")
    if not hist_pt_dir or not hist_pt_dir.IsFolder():
        print("Error: 'HistGivenPt' directory not found in the ROOT file.")
        sys.exit(1)

    pt_keys = hist_pt_dir.GetListOfKeys()
    n_pt_bins = pt_keys.GetEntries()
    print(f"Found {n_pt_bins} pt directories in 'HistGivenPt'.")
    ...
    canvas = ROOT.TCanvas("canvas_pCorr", "pCorrV1 vs pCorrV2 Across Pt Bins", canvas_width, canvas_height)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python plotGivenPt.py <input_root_file> <output_file>")
        sys.exit(1)
